...
func main() {
	if len(os.Args) < 2 {
		...
		fmt.Println("Commands: search, download")
		os.Exit(1)
	}
	// 两个命令集: search, download
	searchCmd := flag.NewFlagSet("search", flag.ExitOnError)
	downloadCmd := flag.NewFlagSet("download", flag.ExitOnError)
	// search 命令集的子命令
	searchTitle := searchCmd.String("title", "", "Movie title to search")
	searchYear := searchCmd.String("year", "", "Movie year (optional)")

	// apiKeyEnv 是 "OMDB_API_KEY", 获取环境变量 OMDB_API_KEY 的值, 作为 --api-key 标志的默认值
	searchAPIKey := searchCmd.String("api-key", os.Getenv(apiKeyEnv), "OMDb API key")
	// download 命令集的子命令
	downloadTitle := downloadCmd.String("title", "", "Movie title to download poster")
	downloadYear := downloadCmd.String("year", "", "Movie year (optional)")
	downloadAPIKey := downloadCmd.String("api-key", os.Getenv(apiKeyEnv), "OMDb API key")

	searchCmd.Usage = func() {
		fmt.Fprintf(searchCmd.Output(), "Usage: %s search --title <title> [--year <year>] [--api-key <key>]\n", os.Args[0])
		fmt.Fprintf(searchCmd.Output(), "Search for movie information\n")
		searchCmd.PrintDefaults()
	}
	downloadCmd.Usage = func() {
		fmt.Fprintf(downloadCmd.Output(), "Usage: %s download --title <title> [--year <year>] [--api-key <key>]\n", os.Args[0])
		fmt.Fprintf(downloadCmd.Output(), "Download movie poster\n")
	...
	case "search":
		...
		if *searchAPIKey == "" {
			fmt.Fprintf(os.Stderr, "Error: OMDb API key is required (use --api-key or %s)\n", apiKeyEnv)
			searchCmd.Usage()
			os.Exit(1)
		}
		if err := searchMovie(*searchTitle, *searchYear, *searchAPIKey); err != nil {
			fmt.Fprintf(os.Stderr, "Error: %v\n", err)
			os.Exit(1)
		}
	case "download":
		...
		if err := downloadPoster(*downloadTitle, *downloadYear, *downloadAPIKey); err != nil {
			fmt.Fprintf(os.Stderr, "Error: %v\n", err)
			os.Exit(1)
		}
	default:
